...
public class Movement
{
	...
	public int choosePath(Room room, Player player) throws SQLException
	{
		...
		while (!path)
		{
			...
			String peekNorth = (room.getExitNorth() != 0) ? temp.getRoomDescription()
					: "There is nothing in this direction.";

			temp = room.getRoom(room.getExitEast());
			String peekEast = (room.getExitEast() != 0) ? temp.getRoomDescription()
					: "There is nothing in this direction.";

			temp = room.getRoom(room.getExitSouth());
			String peekSouth = (room.getExitSouth() != 0) ? temp.getRoomDescription()
					: "There is nothing in this direction.";

			temp = room.getRoom(room.getExitWest());
			String peekWest = (room.getExitWest() != 0) ? temp.getRoomDescription()
					: "There is nothing in this direction.";
